//UBTTask_GetEndPatrolPosition.cpp
...
{
	NodeName = TEXT("GetEndPatrolPosition"); // BehaviorTree에 보일 노드 이름
}
...
EBTNodeResult::Type UBTTask_GetEndPatrolPosition::ExecuteTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
	...
	if(true == NS->GetRandomPointInNavigableRadius(NPC->GetActorLocation(), AIController->PatrolRadius, EndPatrolLocation))
	{
