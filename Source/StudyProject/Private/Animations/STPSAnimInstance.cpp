...
void USTPSAnimInstance::NativeUpdateAnimation(float DeltaSeconds)
{
	...
	if(true == ::IsValid(MovementComponent) && true == ::IsValid(OwnerCharacter))
	{
		...
		// ASTPSCharacter::Move() 함수는 무조건 매 틱마다 호출되는 함수가 아니라 키보드 입력이 있을 때만 매 틱마다 호출된다.
		// 즉, 키보드를 떼면 Move() 함수도 호출되지 않아서 ForwardInputValue가 갱신되지 않는다.
		// 이 때문에 아래와 같이 MovementComponent의 속도를 확인해서 속도가 있을 때만 키보드 입력을 그대로 사용한다.
